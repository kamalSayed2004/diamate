...
export default function MainLayout({
  ...
  const handleLogout = () => {
    ...
    document.cookie =
      "patientId=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
    router.push("/login");
  };
  ...
  return (
    ...
    >
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:relative top-0 left-0 h-full w-64 flex-shrink-0 border-r ${isDark ? "border-blue-800 bg-blue-950/90" : "border-blue-200 bg-white/90"} backdrop-blur-lg flex flex-col z-50 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
      >
        <div
          className={`p-6 flex items-center justify-between gap-3 border-b flex-shrink-0 ${isDark ? "border-blue-800" : "border-blue-200"}`}
        >
          <div className="flex items-center gap-3">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
        ...
        <div
          className={`p-4 border-t ${isDark ? "border-blue-800" : "border-blue-200"} space-y-2`}
        >
          <button
            onClick={toggleTheme}
