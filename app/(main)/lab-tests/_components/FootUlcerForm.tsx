...
export default function FootUlcerForm({
  patientId,
  token,
}: FootUlcerFormProps) {
  ...
  const handleUlcerImageChange = async (
    ...
    <div className="flex flex-col min-h-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div className="md:col-span-2">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5 sm:mb-2">
              Ulcer Image <span className="text-red-500">*</span>
            </label>
            <div className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-6 sm:p-8 text-center bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
              ...
              />
              <label
                htmlFor="ulcer-upload"
                className="cursor-pointer flex flex-col items-center w-full"
              >
                {!ulcerImageBase64 ? (
                  <>
                    <span className="material-icons text-4xl text-slate-400 mb-3 block">
                      cloud_upload
                    </span>
                  ...
                  <div className="relative w-full h-40 md:h-48 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-black/5 flex items-center justify-center">
                    ...
                    <img
                      src={`data:image/jpeg;base64,${ulcerImageBase64}`}
                      alt="Ulcer Preview"
      ...
      <div className="mt-6 sm:mt-8 flex flex-col-reverse sm:flex-row justify-end items-stretch sm:items-center gap-3">
        <button
          ...
          className="px-6 py-2.5 sm:py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-100 text-slate-700 dark:bg-slate-950/50 dark:text-slate-100 hover:bg-slate-200 dark:hover:bg-slate-800 font-semibold text-sm transition-colors w-full sm:w-auto text-center"
        >
          Cancel
        </button>
        <button
