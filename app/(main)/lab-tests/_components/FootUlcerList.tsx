...
export default async function FootUlcerList({
  patientId,
  token,
}: FootUlcerListProps) {
      ...
      {footUlcers.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-500">
          ...
          <p className="font-semibold px-4 text-center">
            No ulcer images found.
          </p>
          <p className="text-xs mt-1 text-center px-6">
            Upload to check with AI.
          </p>
        </div>
      ) : (
            ...
            <div className="mt-3 flex flex-col gap-2">
              ...
              {ulcer.image && (
                <div className="mt-2 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-black/5 flex justify-center items-center h-36 p-1">
                  ...
                  <img
                    src={
                      ulcer.image.startsWith("data:")
                        ? ulcer.image
                        : `data:image/jpeg;base64,${ulcer.image}`
                    }
                    alt="Foot Ulcer"
              ...
              {ulcer.notes && (
                <div className="mt-1 bg-violet-50/50 dark:bg-violet-900/20 border border-violet-100 dark:border-violet-800/50 px-3 py-2 rounded-lg flex items-start gap-2">
                  <span
                    className="material-icons text-[16px] text-violet-500 mt-0.5"
                    aria-hidden="true"
                  >
                    chat_bubble_outline
                  </span>
