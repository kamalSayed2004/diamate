...
import { ChevronLeft, Loader2, Sparkles, User, Heart, ShieldCheck, Camera } from "lucide-react";
...
export default function Register() {
    ...
    switch (name) {
      ...
      case "lastName":
        if (typeof value === "string") {
          if (!value.trim()) {
            ...
            error = "Must contain at least 2 letters and no special characters or numbers.";
          }
        }
        break;
      case "userName":
      ...
      case "password":
        if (typeof value === "string" && !passwordRegex.test(value)) {
          error = "Password must be at least 8 characters, include 1 uppercase letter, 1 lowercase letter, 1 number, and 1 special character (@$!%*?&).";
        }
        break;
      case "dateOfBirth":
        const dob = new Date(value);
        if (dob < new Date("1900-01-01") || dob > new Date("2010-01-01")) {
          error = "Value for DateOfBirth must be between 01/01/1900 and 01/01/2010.";
        }
        break;
      case "weight":
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_API}/Account/RegisterNewUser`, {
        method: "POST",
  ...
  return (
    <div className={`min-h-screen font-[family-name:var(--font-geist-sans)] antialiased transition-colors duration-300 ${isDark ? "bg-slate-950 text-slate-100" : "bg-[#f0f4fa] text-slate-900"}`}>
      <div
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-10 text-center">
          ...
          <p className={`mt-3 ${muted}`}>Join DiaMate and start your journey to smarter health.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <section className={`group relative overflow-hidden rounded-[2rem] border p-6 sm:p-8 shadow-sm transition duration-300 ${surface} ${ringHover}`}>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <input required name="userName" className={getInputClass("userName")} onChange={handleChange} />
                <ValidationError message={validationErrors.userName} />
              </div>
              <div className="space-y-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Password</label>
                <input required name="password" type="password" className={getInputClass("password")} onChange={handleChange} />
                <ValidationError message={validationErrors.password} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Email</label>
                <input required name="email" type="email" className={getInputClass("email")} onChange={handleChange} />
          ...
          <section className={`group relative overflow-hidden rounded-[2rem] border p-6 sm:p-8 shadow-sm transition duration-300 ${surface} ${ringHover}`}>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div className="space-y-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>First Name</label>
                <input required name="firstName" placeholder="e.g. John" className={getInputClass("firstName")} onChange={handleChange} />
                <ValidationError message={validationErrors.firstName} />
              </div>
              <div className="space-y-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Last Name</label>
                <input required name="lastName" placeholder="e.g. Doe" className={getInputClass("lastName")} onChange={handleChange} />
                <ValidationError message={validationErrors.lastName} />
              </div>
              <div className="space-y-2">
                <label className={`text-xs font-semibold uppercase tracking-wider ml-1 ${muted}`}>Date of Birth</label>
                <input required name="dateOfBirth" type="date" className={getInputClass("dateOfBirth")} onChange={handleChange} />
                <ValidationError message={validationErrors.dateOfBirth} />
              </div>
              <div className="space-y-2">
                <label className={`text-xs font-semibold uppercase tracking-wider ml-1 ${muted}`}>Gender</label>
                <select name="gender" className={getInputClass("gender")} onChange={handleChange}>
                  <option value={0}>Male</option>
                  <option value={1}>Female</option>
                </select>
              </div>
              <div className="space-y-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Mobile Phone</label>
                <input required name="phone" placeholder="01012345678" type="tel" maxLength={11} className={getInputClass("phone")} onChange={handleChange} />
                <ValidationError message={validationErrors.phone} />
              </div>
              <div className="space-y-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Home Phone (Optional)</label>
                <input name="homePhone" placeholder="0222345678" type="tel" className={getInputClass("homePhone")} onChange={handleChange} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Full Address</label>
                <input name="address" placeholder="123 Example St, City, Country" className={getInputClass("address")} onChange={handleChange} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Profile Image</label>
                <div className="flex items-center gap-4">
                  <div className={`h-16 w-16 overflow-hidden rounded-full border-2 border-dashed flex items-stretch shrink-0 justify-center ${isDark ? "border-slate-600 bg-slate-800" : "border-slate-300 bg-slate-100"}`}>
                    {formData.profileImage !== "default.png" && formData.profileImage ? (
                      /* eslint-disable-next-line @next/next/no-img-element */
          ...
          <section className={`group relative overflow-hidden rounded-[2rem] border p-6 sm:p-8 shadow-sm transition duration-300 ${surface} ${ringHover}`}>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div className="space-y-2">
                <label className={`text-xs font-semibold uppercase tracking-wider ml-1 ${muted}`}>Diabetes Type</label>
                <select name="diabetesType" className={getInputClass("diabetesType")} onChange={handleChange}>
              ...
              <div className="space-y-2">
                <label className={`text-xs font-semibold uppercase tracking-wider ml-1 ${muted}`}>Diagnosis Date</label>
                <input required name="dateOfDiagnosis" type="date" className={getInputClass("dateOfDiagnosis")} onChange={handleChange} />
                <ValidationError message={validationErrors.dateOfDiagnosis} />
              </div>
              <div className="space-y-2">
                <label className={`text-xs font-semibold uppercase tracking-wider ml-1 ${muted}`}>Weight (kg)</label>
                <input name="weight" type="number" defaultValue={70} className={getInputClass("weight")} onChange={handleChange} />
                <ValidationError message={validationErrors.weight} />
              </div>
              <div className="space-y-2">
                <label className={`text-xs font-semibold uppercase tracking-wider ml-1 ${muted}`}>Height (cm)</label>
                <input name="height" type="number" defaultValue={170} className={getInputClass("height")} onChange={handleChange} />
                <ValidationError message={validationErrors.height} />
              </div>
              <div className="space-y-2 sm:col-span-2">
                <label className={`text-sm font-medium ml-1 ${muted}`}>Medical Notes / Allergies</label>
                <textarea
                  name="notes"
          ...
          <div className="pt-4">
            ...
            >
              {loading ? (
                ...
                <>
                  Complete Registration <Sparkles className="h-5 w-5 transition-transform group-hover:-rotate-12" />
                </>
              )}
            </button>
          </div>
